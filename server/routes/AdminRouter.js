...
router.get('/feed/:category', (req, res)=>{
	Feed.getFeed(req.params.category).then(feed => res.json({message: "Feed based on the category", success: true, feed: feed}));
});

router.post('/feed/update/story',(req, res) => {
	Feed.updateStory(req.body.storyId, req.body.category).then(doc => res.json({message: "Story Updated!", success: true, story: doc}));
});

router.delete('/feed/delete/:storyId', (req, res)=>{
	let result = Feed.deleteStory(req.params.storyId);
	if(result == undefined){
		res.json({message: "story deleted", success: true});
	} else {
		res.json({message: result, error: true});
	}
});
